...
open(F, "/tmp/tor-build/tor-0.3.4.9/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/tmp/tor-build/tor-0.3.4.9/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/tmp/tor-build/tor-0.3.4.9/doc/tor.1.txt") or die;
while (<F>) {
